...
const Stat = ({address}:{address:string}) => {
    ...
    const fetchData = async (add:string) => {
        try {
            ...
            const num_contracts = await fetch("https://api.tzkt.io/v1/accounts/"+add);
            ...
            const all_tx = await fetch("https://api.tzkt.io/v1/operations/transactions?limit=10&anyof.sender.target="+add);
            ...
            setData({
                ...
                tx_vol:  Math.round((accountData.total_received + accountData.total_sent)*0.62),
                ...
                tx_sent_list: sentTxData.map(item => ({
                    address: item.target.address,
                    amount: item.amount,
                    timestamp: item.timestamp
                })),
                tx_received_list: receivedtTxData.map(item => ({
                    address: item.sender.address,
                    amount: item.amount,
                    timestamp: item.timestamp
                })),
                tx_all_list: allTxData.map(item => ({
    ...
    const contracts = {
        labels: ['tz2K...Js', 'tz1T...Rz', 'tz1R...vj', 'tz1T...Rz', 'tz1R...vj', 'tz8S...Yt'],
        datasets: [
          {
            label: 'Tx Data',
            data: [5, 2, 1, 3, 2, 4],
    ...
    <div className=" mt-4">
        <div className="flex justify-around items-center text-white">
            ...
            <div className="h-[8vh] w-[30vw] border-solid border-white border-2 rounded-xl bg-[#4942e4] opacity-75 m-1 relative">
                ...
                <div  className="text-xl absolute bottom-[10%] right-[10%]">
                    0.14 <span className="text-xs">Tzk</span>
                </div>
            </div>
            <div className="h-[8vh] w-[30vw] border-solid border-white border-2 rounded-xl bg-[#4942e4] opacity-75 m-1 relative">
                <div className="text-xs absolute top-[10%] left-[5%]">
                ...
                <div  className="text-xl absolute bottom-[10%] right-[10%]">
                    {data.tx_vol} <span className="text-xs">Tzk</span>
                </div>
            </div>
        </div>
